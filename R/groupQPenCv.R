...
#' @param gamma tuning parameter in the Huber loss
...
#' \item{cv_min}{The value of CV error corresponding to \code{lambda.min}.}
#' \item{cv_1se}{The value of CV error corresponding to \code{lambda.1se}.}
...
#' \item{lambda_min_eachtau}{Optimal lambda.min value with respect to each quantile.}
#' \item{lambda_1se_eachtau}{Optimal lambda.1se value with respect to each quantile.}
#' \item{cv_min_eachtau}{The value of CV errors for all taus corresponding to \code{lambda_min_eachtau}.}
#' \item{cv_1se_eachtau}{The value of CV errors for all taus corresponding to \code{lambda_1se_eachtau}.}
#' \item{cvup_eachtau}{CV error + 1 standard error, for each tau}
#' \item{cvlo_eachtau}{CV error - 1 standard error, for each tau}
#'  
#' @export
#'
#' @examples
...
#' cvfit$cv_all
#' 
cv.hrq_tau_glasso<- function(model_obj=NULL, nfolds=5, loss="rq", wt_tau_loss=NULL, x=NULL, y=NULL, tau=NULL, folds=NULL, gamma=NULL, ...){
  ...
  if(!is.null(model_obj)){
    ...
    fullmodel<- hrq_tau_glasso(x=x, y=y, tau=tau, gamma=gamma, ...)
  ...
  for(i in 1:nfolds){
    ...
    train_model<- hrq_tau_glasso(x=train_x, y=train_y, tau=tau, gamma=gamma, lambda=lambda, lambda.discard=FALSE, ...) #,...
    pred<- predict.hrq_tau_glasso(train_model, newX = test_x)
    if(loss == "se"){
      se<- sapply(1:nlambda, function(xx) (pred[,xx+1]-rep(test_y,ntau))^2)
      mse[,i]<- as.vector(sapply(1:nlambda, function(xx) tapply(se[,xx], rep(1:ntau, each=nrow(test_x)), mean))) 
    } 
    if(loss == "rq"){
      eq<- sapply(1:nlambda, function(xx) rq.loss.aug(rep(test_y,ntau)-pred[,xx+1], tau, n=nrow(test_x)))
  ...
  # cv.mse<- tapply(apply(mse, 1, mean), rep(1:nlambda, each=ntau), mean)
  # cv.mqe<- tapply(apply(mqe, 1, mean), rep(1:nlambda, each=ntau), mean)
  ...
  if(loss == "rq"){
    ...
    cv.1se <- cv.mqe[,sapply(1:length(lambda.1se), function(xx) which(lambda==lambda.1se[xx]))]
    # cv.ind.min<- which(cv.mqe[med.ind,]==min(cv.mqe[med.ind,]))
    # cv.ind.1se<- which(cv.mqe[med.ind,]<=min(cv.mqe[med.ind,])+cv.mqe.1se[med.ind, cv.ind.min])
    ...
    output<- list(beta=fullmodel$beta, lambda=lambda, cv_all= cv.mqe, err_all=mqe,
                  ...
                  lambda_min_eachtau=lambda.min, lambda_1se_eachtau=lambda.1se, cv_min_eachtau=cv.min, cv_1se_eachtau=cv.1se, 
                  cvup_eachtau=cv.mqe+cv.mqe.1se, cvlo_eachtau=cv.mqe-cv.mqe.1se, 
                  folds=folds, ntau=ntau, n.nonzero.beta=fullmodel$n.nonzero.beta, p=ncol(test_x), tau=fullmodel$tau)
    ...
    if(loss=="se"){
      ...
      cv.1se <- cv.mse[,sapply(1:length(lambda.1se), function(xx) which(lambda==lambda.1se[xx]))]
      # cv.ind.min<- which(cv.mse[med.ind,]==min(cv.mse[med.ind,]))
      # cv.ind.1se<- which(cv.mse[med.ind,]<=min(cv.mse[med.ind,])+cv.mse.1se[med.ind, cv.ind.min])
      # lambda.1se<- max(lambda[cv.ind.1se])
      ...
      output<- list(beta=fullmodel$beta, lambda=lambda, cv_all= cv.mse, err_all=mse,
                    ...
                    lambda_min_eachtau=lambda.min, lambda_1se_eachtau=lambda.1se, cv_min_eachtau=cv.min, cv_1se_eachtau=cv.1se, 
                    cvup_eachtau=cv.mse+cv.mse.1se, cvlo_eachtau=cv.mse-cv.mse.1se, 
                    folds=folds, n.nonzero.beta=fullmodel$n.nonzero.beta, ntau=ntau, p=ncol(test_x), tau=fullmodel$tau)
      class(output) <- "cv.hrq_tau_glasso"
      return(output)
    }else{
...
coef.cv.hrq_tau_glasso<- function(cv.fit, s){
  ...
  if(length(sval)==1){
    ...
    rownames(beta)<- c("Intercept", paste("V", 1:cv.fit$p, sep = ""))
    ...
    for(i in 1:length(ind)){
      ...
      rownames(beta[[i]])<- c("Intercept", paste("V", 1:cv.fit$p, sep = ""))
