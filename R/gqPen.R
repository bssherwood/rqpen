...
gq.pen.old <- function(x, y, tau, lambda = NULL, weight_obs = NULL, weight_lam = NULL, 
                        weight_tau = NULL, gmma = 0.2, maxIter = 200, lambda.discard = TRUE, 
                        epsilon = 1e-04, beta0 = NULL) 
{
  ntau <- length(tau)
  np <- dim(x)
  n <- np[1]
  ...
  if (is.null(weight_lam)) 
    weight_lam <- sqrt(nng)
  if (is.null(weight_obs)) 
    weight_obs <- rep(1, n)
  if (is.null(weight_tau)) 
    weight_tau <- rep(1, ntau)
  Xaug <- kronecker(diag(ntau), x)
  groupIndex <- rep(1:p, ntau)
  groupOrder <- order(groupIndex)
  Xaug <- Matrix(Xaug[, groupOrder])
  ...
  if (is.null(beta0)) {
    ...
    gmma.max <- 4
    gmma <- min(gmma.max, max(gmma0, quantile(abs(r), probs = 0.1)))
    beta0 <- c(t(rbind(b.int, matrix(0, p, ntau))))
  }
  else {
    r <- Yaug - c(cbind(1, x) %*% beta0)
    beta0 <- c(t(beta0))
    dev0 <- sum(rq.loss.aug(r, tau, n))
    gmma.max <- 4
    gmma <- min(gmma.max, max(gmma0, quantile(abs(r), probs = 0.1)))
  ...
  r0 <- r
  grad_k <- -neg_gradient_aug(r0, weight_obs_aug, tau_aug, 
                              gmma, Xaug, ntau)
  grad_k.norm <- tapply(grad_k, groupIndex, weighted_norm, 
                        normweights = weight_tau)
  lambda.max <- max(grad_k.norm/weight_lam)
  lambda.flag <- 0
  if (is.null(lambda)) {
    lambda.min <- ifelse(n > p, lambda.max * 0.001, lambda.max * 
                           0.01)
    lambda <- exp(seq(log(lambda.max), log(lambda.min), length.out = 101))
  }
  else {
    lambda.discard <- FALSE
    if (lambda.max > max(lambda)) {
      lambda <- exp(c(log(lambda.max), log(sort(lambda, 
                                                decreasing = TRUE))))
    }
    else {
      if (length(lambda) > 1 & min(lambda) < lambda.max) {
        lambda.flag <- 1
        lambda.user <- lambda
        lambda <- exp(c(log(lambda.max), log(sort(lambda[lambda < 
                                                           lambda.max], decreasing = TRUE))))
      }
      else {
        betaEst <- rbind(b.int, matrix(0, p, ntau))
        rownames(betaEst) <- c("Intercept", paste("V", 
                                                  1:p, sep = ""))
        colnames(betaEst) <- paste("tau_", tau, sep = "")
        return(list(beta = betaEst))
      }
  ...
  if (length(lambda) == 2) {
    lambda <- lambda[2]
    update <- solvebetaCpp(Xaug, Yaug, n, tau_aug, gmma, 
                           weight_obs_aug, groupIndex, lambda, weight_lam, weight_tau, 
                           eigen.sub.H, beta0, maxIter, epsilon, ntau)
    beta1 <- update$beta_update
    iter_num <- update$n_iter
    converge_status <- update$converge
    update.r <- as.vector(update$resid)
    dev1 <- sum(weight_obs_aug * rq.loss.aug(update.r, tau, 
                                             n))
    pen.loss <- dev1/n + lambda * sum(eigen.sub.H * sapply(1:p, 
                                                           function(xx) l2norm(beta1[-(1:ntau)][groupIndex == 
                                                                                                  xx])))
    group.index.out <- unique(groupIndex[beta1[-(1:ntau)] != 
                                           0])
    betaEst <- matrix(beta1, (p + 1), ntau, byrow = T)
    colnames(betaEst) <- paste("tau_", tau, sep = "")
    if (is.null(colnames(x))) {
      rownames(betaEst) <- c("Intercept", paste("V", 1:p, 
                                                sep = ""))
    }
    else {
      rownames(betaEst) <- c("Intercept", colnames(x))
    }
    output <- list(beta = betaEst, lambda = lambda, null.dev = dev0, 
                   pen.loss = pen.loss, loss = dev1/n, tau = tau, n.nonzero.beta = length(group.index.out), 
                   index.nonzero.beta = group.index.out, X = x, y = y)
    output.hide <- list(converge = update$converge, iter = update$iter, 
                        rel_dev = dev1/dev0)
    class(output) <- "hrq_tau_glasso"
    return(output)
  }
    ...
    group.index.out <- matrix(0, p, length(lambda))
    n.grp <- rep(0, length(lambda))
    n.grp[1] <- 0
    beta.all <- matrix(0, (p + 1) * ntau, length(lambda))
    beta.all[, 1] <- beta0
    kkt_seq <- rep(NA, length(lambda))
    kkt_seq[1] <- TRUE
    converge <- rep(0, length(lambda))
    converge[1] <- TRUE
    iter <- rep(0, length(lambda))
    iter[1] <- 1
    outer_iter_count <- rep(0, length(lambda))
    outer_iter_count[1] <- 0
    rel_dev <- rep(0, length(lambda))
    rel_dev[1] <- 1
    loss <- rep(0, length(lambda))
    loss[1] <- dev0/n
    pen.loss <- rep(0, length(lambda))
    pen.loss[1] <- dev0/n
    gmma.seq <- rep(0, length(lambda))
    gmma.seq[1] <- gmma
    active.ind <- NULL
    for (j in 2:length(lambda)) {
      if (length(active.ind) < p) {
        grad_k <- -neg_gradient_aug(r0, weight_obs_aug, 
                                    tau_aug, gmma, Xaug, ntau)
        grad_k.norm <- tapply(grad_k, groupIndex, l2norm)
        active.ind <- which(grad_k.norm >= weight_lam * 
                              (2 * lambda[j] - lambda[j - 1]))
        n.active.ind <- length(active.ind)
        if (length(active.ind) == p) {
          outer_iter <- 0
          kkt_met <- NA
          max_iter <- 50
          update <- solvebetaCpp(Xaug, Yaug, n, tau_aug, 
                                 gmma, weight_obs_aug, groupIndex, lambda[j], 
                                 weight_lam, weight_tau, eigen.sub.H, beta0, 
                                 maxIter, epsilon, ntau)
          beta0 <- update$beta_update
          update.iter <- update$n_iter
          update.converge <- update$converge
        ...
        if (length(active.ind) == 0) {
          ...
          while (!kkt_met & length(inactive.ind) > 0) {
            ...
            x.sub <- Xaug[, reduced.ind]
            reduced.eigen <- eigen.sub.H[u.reduced.group.index]
            update <- solvebetaCpp(x.sub, Yaug, n, tau_aug, 
                                   gmma, weight_obs_aug, reduced.group.index, 
                                   lambda[j], weight_lam, weight_tau, eigen.sub.H, 
                                   beta0[c(1:ntau, reduced.ind + ntau)], maxIter, 
                                   epsilon, ntau)
            beta.update <- update$beta_update
            update.r <- as.vector(update$resid)
            update.converge <- update$converge
            ...
            update.grad <- update$gradient
            beta0[c(1:ntau, reduced.ind + ntau)] <- beta.update
            kkt_results <- kkt_check_aug(r = update.r, 
                                         weights = weight_obs_aug, w = weight_lam, 
                                         gmma = gmma, tau = tau_aug, group.index = groupIndex, 
                                         inactive.ind = inactive.ind, lambda = lambda[j], 
                                         x = Xaug, ntau = ntau)
            if (kkt_results$kkt == TRUE) {
              kkt_met <- TRUE
            }
        ...
        kkt_met <- NA
        max_iter <- 50
        update <- solvebetaCpp(x = Xaug, y = Yaug, n = n, 
                               tau = tau_aug, gmma = gmma, weights = weight_obs_aug, 
                               groupIndex = groupIndex, lambdaj = lambda[j], 
                               wlambda = weight_lam, wtau = weight_tau, eigenval = eigen.sub.H, 
                               betaini = beta0, maxIter = maxIter, epsilon = epsilon, 
                               ntau = ntau)
        beta0 <- update$beta_update
        update.r <- as.vector(update$resid)
        update.converge <- update$converge
      ...
      r0 <- update.r
      gmma.max <- 4
      gmma <- min(gmma.max, max(gmma0, quantile(abs(r0), 
                                                probs = 0.1)))
      grp.ind <- unique(groupIndex[beta0[-(1:ntau)] != 
                                     0])
      group.index.out[grp.ind, j] <- grp.ind
      n.grp[j] <- length(grp.ind)
      dev1 <- sum(weight_obs_aug * rq.loss.aug(r0, tau, 
                                               n))
      loss[j] <- dev1/n
      pen.loss[j] <- dev1/n + lambda[j] * sum(eigen.sub.H * 
                                                sapply(1:p, function(xx) l2norm(beta0[-(1:ntau)][groupIndex == 
                                                                                                   xx])))
      rel_dev[j] <- dev1/dev0
      rel_dev_change <- rel_dev[j] - rel_dev[j - 1]
      if (abs(rel_dev_change) < 0.001 & j > 70 & lambda.discard) 
        break
    }
    stop.ind <- which(rel_dev != 0)
    if (length(stop.ind) == 0) 
      stop.ind <- 1:length(lambda)
    if (lambda.flag == 0) {
      stop.ind <- stop.ind[-1]
      beta.final <- beta.all[, stop.ind]
      if (is.null(colnames(x))) {
        rownames(beta.final) <- c(paste("Intercept(tau=", 
                                        tau, ")", sep = ""), rep(paste("V", 1:p, sep = ""), 
                                                                 each = ntau))
      }
      else {
        rownames(beta.final) <- c(paste("Intercept(tau=", 
                                        tau, ")", sep = ""), rep(colnames(x), each = ntau))
      }
      output <- list(beta = Matrix(beta.final), lambda = lambda[stop.ind], 
                     null.dev = dev0, pen.loss = pen.loss[stop.ind], 
                     loss = loss[stop.ind], n.nonzero.beta = n.grp[stop.ind], 
                     index.nonzero.beta = Matrix(group.index.out[, 
                                                                 stop.ind]), tau = tau, X = x, y = y)
      output_hide <- list(iter = iter[stop.ind], rel_dev = rel_dev[stop.ind], 
                          outer.iter = outer_iter_count[stop.ind], kkt = kkt_seq[stop.ind], 
                          gmma = gmma.seq[stop.ind])
      class(output) <- "hrq_tau_glasso"
      return(output)
    }
    if (lambda.flag == 1) {
      length.diff <- length(lambda.user) - length(lambda) + 
        1
      beta.all <- cbind(matrix(beta.all[, 1], nrow = ntau * 
                                 (p + 1), ncol = length.diff), beta.all[, -1])
      beta.final <- beta.all
      if (is.null(colnames(x))) {
        rownames(beta.final) <- c(paste("Intercept(tau=", 
                                        tau, ")", sep = ""), rep(paste("V", 1:p, sep = ""), 
      ...
      output <- list(beta = Matrix(beta.final), lambda = lambda.user, 
                     null.dev = dev0, pen.loss = c(rep(pen.loss[1], 
                                                       length.diff), pen.loss[-1]), loss = c(rep(loss[1], 
                                                                                                 length.diff), loss[-1]), n.nonzero.beta = c(rep(n.grp[1], 
                                                                                                                                                 length.diff), n.grp[-1]), index.nonzero.beta = Matrix(cbind(matrix(group.index.out[, 
                                                                                                                                                                                                                                    1], nrow = nrow(group.index.out), ncol = length.diff), 
                                                                                                                                                                                                             group.index.out[, -1])), tau = tau, X = x, 
                     y = y)
      output_hide <- list(iter = c(rep(iter[1], length.diff), 
                                   iter), rel_dev = c(rep(rel_dev[1], length.diff), 
                                                      rel_dev), outer.iter = outer_iter_count[stop.ind], 
                          kkt = c(rep(kkt_seq[1], length.diff), kkt_seq[-1]), 
                          gmma = c(rep(gmma.seq[1], length.diff), gmma.seq[-1]))
      class(output) <- "hrq_tau_glasso"
      warning(paste("first ", length.diff, " lambdas results in pure sparse estimates!", 
                    sep = ""))
      return(output)
    }
  }
