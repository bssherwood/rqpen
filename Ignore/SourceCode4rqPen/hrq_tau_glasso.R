...
#' 
hrq_tau_glasso<- function(x, y, tau, lambda=NULL, weight_obs=NULL, weight_lam=NULL, weight_tau=NULL, gamma=0.2, 
  ...
  if(scalex){
    temp <- sum(weight_obs*x[,1]^2)/(n*gamma)
    eigen.sub.H <- rep(temp, p)
  }else{
    eigen.sub.H <- colSums(weight_obs*x^2)/(n*gamma)
  }
  ...
  if(length(lambda)==2){
    lambda<- lambda[2]
    update<- solvebetaCpp(Xaug, Yaug, n, tau_aug, gamma, weight_obs_aug, groupIndex, lambda, weight_lam, 
      ...
      if(length(active.ind)<p){
        ...
        if(length(active.ind)==0){
          ...
          while(!kkt_met & length(inactive.ind)>0){
            ...
            kkt_results <- kkt_check_aug(r=update.r,weights=weight_obs_aug,w=weight_lam,gamma=gamma,tau=tau_aug,group.index=groupIndex,
