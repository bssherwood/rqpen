...
rq.huber.aug<- function(r, tau, gamma, n){
  tau.vec <- rep(tau, each=n)
  r<- as.vector(r)
  (huber.loss(r,gamma)+(2*tau.vec-1)*r)/2
}
##############################
...
rq.huber.deriv.aug<- function(r, tau, gamma, n){
  tau.vec <- rep(tau, each=n)
  r<- as.vector(r)
  le.ind<- which(abs(r) <= gamma)
  if(length(le.ind)!= 0){
    l.vec<- r
    l.vec[le.ind]<- (r[le.ind]/gamma+(2*tau.vec[le.ind]-1))/2
...
rq.tanh.deriv.aug<- function(r, tau, gamma, n){
  tau.vec <- rep(tau, each=n)
  (tanh(r/gamma)+2*tau.vec-1)/2
} # end of function
############################
...
neg.gradient.aug <- function(r,weights,tau,gamma,x,n,apprx){  # input r, weights, x needs to be the augmented version
  if(apprx=="huber"){
    wt_deriv <- as.vector(weights*rq.huber.deriv.aug(r, tau, gamma, n))
  }else{
    wt_deriv <- as.vector(weights*rq.tanh.deriv.aug(r, tau, gamma, n))
  }
...
kkt_check.aug<- function(r, weights, w, gamma, tau, group.index, inactive.ind, lambda, x, n, apprx){
  grad<- -neg.gradient.aug(r, weights, tau, gamma, x, n, apprx)
  grad.norm<- tapply(grad, group.index, l2norm)/w
  bad_spots <- grad.norm > lambda
  if(sum(bad_spots[inactive.ind])==0){
    list(kkt=TRUE,new_groups=NULL)
